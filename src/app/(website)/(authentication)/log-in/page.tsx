...
const Login = () => {
    ...
    try {
      ...
      if(handleLoginPatron.fulfilled.match(resultAction)){
        router.push('/')
         ...
         if (typeof resultAction.payload === 'string' && resultAction.payload.includes('auth/user-not-found')) {
          setError('password', {
            type: 'manual',
          ...
          typeof resultAction.payload === 'string' && resultAction.payload.includes('auth/wrong-password')
        ) {
          setError('password', {
            type: 'manual',
  ...
  return (
    <Box className='content-center' style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: '#F9FAFB' }}>
      <AuthIllustrationV1Wrapper>
        <Card>
          <CardContent style={{ padding: '2rem 2.5rem' }}>
            <Box style={{ marginBottom: '2rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Link href='/'>
                      ...
                      InputProps={{
                        endAdornment: (
                          <InputAdornment position='end'>
                            ...
                            >
                              <IconifyIcon fontSize='1.25rem' icon={showPassword ? 'tabler:eye' : 'tabler:eye-off'} />
                            </IconButton>
                          </InputAdornment>
                        )
                      }}
...
>
  {loading ? <CircularProgress size={24} color="inherit" /> : 'Log in'}
              ...
              <Box style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Typography style={{ color: '#6B7280', marginRight: '0.5rem' }}>New on our platform?</Typography>
                <Typography component={LinkStyled} href='/sign-up'>
                  Create an account
                </Typography>
              </Box>
              <Divider style={{ color: '#9CA3AF', fontSize: '0.8125rem', margin: '1.5rem 0' }}>or</Divider>
