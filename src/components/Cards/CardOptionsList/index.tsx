...
export default function CardOptionaList({ data } : { data: DonationOptionsProps[]}) {
  ...
  const RowOptions = ({ patronID, data}: { patronID: string, data: EditDonationOptionType}) => {
      ...
      MySwal.fire({
        ...
        if (result.value) {
          ...
          MySwal.fire({
            icon: 'success',
            title: 'Deleted!',
            text: 'Donation Option has been deleted.',
            customClass: {
    ...
    return (
      ...
      <>
        {/* Edit Modal */}
        <EditDonationOptionDrawer open={editOptionOpen} toggle={toggleEditOptionDrawer} OptionData={data} />  
      </>
      </>
    )
  }
  ...
  return (
    <Paper sx={{ p: 2, mb: 4, borderRadius: 2, boxShadow: 3 }}>
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
        ...
        <Button variant="contained" color="primary" size="small" onClick={toggleAddNewDonationOption}>
          Add New Option
        </Button>
      </Box>
      <Table>
